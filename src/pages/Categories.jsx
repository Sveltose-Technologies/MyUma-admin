...
const Categories = () => {
  ...
  // --- FRONTEND ONLY LOGIC FOR FEATURED ---
  const [featuredIds, setFeaturedIds] = useState(() => {
    const saved = localStorage.getItem("local_featured_categories");
    return saved ? JSON.parse(saved) : [];
  });

  // Merge API data with LocalStorage featured status
  const enrichedCategories = useMemo(() => {
    return categories.map((cat) => ({
      ...cat,
      featured: featuredIds.includes(cat._id),
    }));
  }, [categories, featuredIds]);

  const pagination = usePagination(enrichedCategories, 10);

  const [showModal, setShowModal] = useState(false);
  const [editId, setEditId] = useState(null);
  const [name, setName] = useState("");
  const [featured, setFeatured] = useState(false); // Checkbox state
  ...
  // Update LocalStorage whenever featuredIds change
  useEffect(() => {
    localStorage.setItem(
      "local_featured_categories",
  ...
  const handleSave = async (e) => {
    ...
    // 1. Call standard API (Backend ignores "featured")
    const result = editId
      ? await updateItem(editId, formData)
      : await addItem(formData);

    // 2. Handle Featured logic locally after API success
    if (result) {
      const savedId = editId || result._id || result.data?._id;

      if (savedId) {
        if (featured) {
          // Add to featured list if not already there
          setFeaturedIds((prev) => [...new Set([...prev, savedId])]);
        } else {
          // Remove from featured list
          setFeaturedIds((prev) => prev.filter((id) => id !== savedId));
        }
      }
      handleCloseModal();
    }
  ...
  const openEditModal = (item) => {
    setEditId(item._id);
    setName(item.name);
    setFeatured(featuredIds.includes(item._id)); // Check local list
    ...
    <div className="container-fluid py-3 py-md-4">
      <div className="d-flex flex-column flex-sm-row justify-content-between align-items-center gap-3 mb-4">
        <div>
          ...
          <p className="text-muted small m-0">
            Manage website categories (Featured status managed locally)
          </p>
        </div>
                  ...
                  <td className="text-end px-4">
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        ); // Cleanup local storage on delete
                      }}
                      style={{ borderRadius: "8px" }}>
                      <i className="bi bi-trash3 text-danger"></i>
                    </button>
