...
const Reviews = () => {
  // 1. CRUD Hook (addItem removed to disable Create functionality)
  const {
    data: rawReviews,
    loading,
    fetchAll,
  ...
  const loadHelpers = async () => {
    try {
      ...
      // Match your JSON keys: "auths" and "listings"
      setUsers(uRes?.auths || uRes?.data || []);
      setItems(iRes?.listings || iRes?.data || []);
    } catch (err) {
      console.error("Failed to load helper data", err);
    }
  ...
  // 2. Data Mapping: Matches review IDs to User FullNames and Listing Titles
  const mappedReviews = useMemo(() => {
    // rawReviews might be an array directly or inside a data object
    const reviewsArr = Array.isArray(rawReviews)
      ? rawReviews
      : rawReviews?.data || [];

    return reviewsArr.map((review) => {
      const userObj = users.find(
        (u) => String(u._id) === String(review.userId),
      );
      const itemObj = items.find(
        (i) => String(i._id) === String(review.itemId),
      );

      return {
        ...review,
        userName: userObj ? userObj.fullName : "Unknown User",
        itemName: itemObj ? itemObj.title : "Unknown Listing",
  ...
  return (
    <div className="container-fluid py-4">
      ...
      <div className="mb-4">
        <h4 className="fw-bold m-0 text-navy">Manage Reviews</h4>
        <p className="text-muted small m-0">Monitor and update user feedback</p>
      </div>
          ...
          <table className="table table-hover align-middle mb-0 text-nowrap">
            <thead className="bg-navy text-white">
              <tr>
                ...
                <th className="py-3">Listing Title</th>
                <th className="py-3">Comment</th>
                <th className="text-end px-4 py-3">Actions</th>
              </tr>
            </thead>
              ...
              {loading && mappedReviews.length === 0 ? (
                ...
                pagination.paginatedData.map((item, index) => (
                  <tr key={item._id}>
                    ...
                    <td>
                      <div className="fw-bold text-navy">{item.userName}</div>
                      <div className="text-muted small">{item.userEmail}</div>
                    </td>
                    <td>
                      <span className="badge bg-black text-navy border px-2 py-1 fw-normal">
                        {item.itemName}
                      </span>
                    </td>
                    <td
        ...
        <div
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content border-0 shadow rounded-4 overflow-hidden">
              <div className="modal-header border-0 p-4 pb-0">
                <h5 className="fw-bold m-0 text-navy">Edit User Review</h5>
                <button
                  type="button"
                  className="btn-close shadow-none"
                  onClick={handleClose}></button>
              ...
              <form onSubmit={handleSave}>
                <div className="modal-body p-4">
                  ...
                  <div>
                    <label className="small fw-bold text-muted">COMMENT</label>
                    <textarea
