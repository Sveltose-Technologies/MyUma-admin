...
import { useNavigate } from "react-router-dom";
import { toast } from "react-toastify";
import { verifyOtpApi, resendOtpApi } from "../services/authService"; // Import resend API
import { setLoading, setLogin } from "../store/slices/authSlice";
import CustomButton from "../components/common/CustomButton";
...
const VerifyOtp = () => {
  ...
  const handleResend = async () => {
    ...
    console.log("Resending OTP for:", tempEmail);

    if (!tempEmail) {
      toast.error("Email session expired. Please login again.");
      navigate("/login");
      return;
    }

    try {
      const response = await resendOtpApi(tempEmail);
          ...
          <div className="text-center mt-3">
            {canResend ? (
              ...
              <p className="small text-muted mb-0">
                Resend code in{" "}
                <span className="fw-bold text-dark">{timer}s</span>
              </p>
            )}
          </div>
